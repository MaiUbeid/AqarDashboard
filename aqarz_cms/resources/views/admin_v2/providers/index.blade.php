          ...
          datatable = $('#kt_category_list_datatable').KTDatatable({
            ...
            rows: {
              afterTemplate: function (element, data, rowNumber) {
                element.find('#is_certified').on('change', function (event) {



                  var element = $(this);
                  var id = element.attr('data-id');
                  var url = '{{route('admin.CertifiedUser',':id')}}';
                  ...
                  $.ajax({
                    ...
                    .done(function (data) {
                      ...
                      if (data.status == true) {
                        ...
                        toastr.warning('@lang('تمت العملية بنجاح')');
            ...
            columns: [
...

              {
                ...
                template: function (row, index, datatable) {
                  ...
                  {
                    return '<span class="class="btn btn-danger"">لا</span>';
                  }
                },
              },
...

              {
                ...
                template: function (row, index, datatable) {
                  ...
                  return   '<div class="col-md-12 kt-margin-b-20-tablet-and-mobile">' +
                          '<div class="form-group">' +
                          '<label class="kt-checkbox">\n' +
                          '<input data-id="' + row.id + '" id="is_certified" '+check+' type="checkbox">' +
                          '<span></span>\n' +
                          '</label>'+
                          '</div>' +
                          ' </div>';
...

              {
                ...
                template: function (row) {
                  ...
                  var edit_url = '{{$edit_url}}'.replace('NaN', row.id);
                  var active_url = '{{$active_url}}'.replace('NaN', row.id);
                ...
                <div class="dropdown-menu dropdown-menu-right">\
                  <ul class="kt-nav">\
                    <li class="kt-nav__item">\
                      ...
                      \ <a href="' + active_url + '" class="kt-nav__link">\
                        <i class="kt-nav__link-icon flaticon2-contract"></i>\
                        <span class="kt-nav__link-text">{{__('تفعيل')}}</span>\
                      </a>\
                    </li>\
                  </ul>\
                </div>\
