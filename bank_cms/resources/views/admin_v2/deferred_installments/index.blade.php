        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form method="post" action="{{route('admin.updateDeferredStatus')}}" >
                    ...
                    <div class="kt-form__actions">
                        <div class="row">
                            ...
                            <div class="col-lg-9 col-xl-6">
                                ...
                                <a class="btn btn-clean btn-bold" href="{{route('admin.deferred_installments.index')}}">{{__('views.Cancel')}}</a>
                            </div>
                        </div>
                    </div>
...
@push('scripts')
    <script>
        ...
        var init = function () {
          ...
          datatable = $('#kt_category_list_datatable').KTDatatable({
            ...
            rows: {
              afterTemplate: function (element, data, rowNumber) {
                element.find('#request-model').on('click', function (event) {
                  ...
                  var url = '{{route('admin.getdeferredRequest',':id')}}';
                            ...
                            if(value=='is_salary_adapter_on')
                            {
                              if(key=='0')
                              {
                                str += '<div class="col-md-12 kt-margin-b-20-tablet-and-mobile">' +
                                  ...
                                  '<input readonly="" value="لا" type="text" class="form-control" id="kt_datepicker_1" readonly="" >' +
                                  '</div> </div>';
                              }
                              else if (key=='1')
                              {
                                str += '<div class="col-md-12 kt-margin-b-20-tablet-and-mobile">' +
                                  '<div class="form-group">' +
                                  '<label>' + translation(value) + '</label>' +
                                  '<input readonly="" value="نعم" type="text" class="form-control" id="kt_datepicker_1" readonly="" >' +
                                  '</div> </div>';
                              }
                            }


                           else if(value=='tenant_job_type')
                            {
                              if(key=='governmental')
                              {
                                str += '<div class="col-md-12 kt-margin-b-20-tablet-and-mobile">' +
                                  '<div class="form-group">' +
                                  '<label>' + translation(value) + '</label>' +
                                  '<input readonly="" value="حكومي" type="text" class="form-control" id="kt_datepicker_1" readonly="" >' +
                                  '</div> </div>';
                              }
                              else if (key=='special')
                              {
                                str += '<div class="col-md-12 kt-margin-b-20-tablet-and-mobile">' +
                                  '<div class="form-group">' +
                                  '<label>' + translation(value) + '</label>' +
                                  '<input readonly="" value="خاص" type="text" class="form-control" id="kt_datepicker_1" readonly="" >' +
                                  '</div> </div>';
                              }
                              else if (key=='soldier')
                              {
                                str += '<div class="col-md-12 kt-margin-b-20-tablet-and-mobile">' +
                                  '<div class="form-group">' +
                                  '<label>' + translation(value) + '</label>' +
                                  '<input readonly="" value="جيش" type="text" class="form-control" id="kt_datepicker_1" readonly="" >' +
                                  '</div> </div>';
                              }
                            }
                            ...
                            else if(value=='status')
                            {
                              if(key=='0')
                              {
                                str += '<div class="col-md-12 kt-margin-b-20-tablet-and-mobile">' +
                                  '<div class="form-group">' +
                                  '<label>' + translation(value) + '</label>' +
                                  '<input readonly="" value="ملغي" type="text" class="form-control" id="kt_datepicker_1" readonly="" >' +
                                  '</div> </div>';
                              }
                              else if (key=='1')
                              {
                                str += '<div class="col-md-12 kt-margin-b-20-tablet-and-mobile">' +
                                  '<div class="form-group">' +
                                  '<label>' + translation(value) + '</label>' +
                                  '<input readonly="" value="مقبول" type="text" class="form-control" id="kt_datepicker_1" readonly="" >' +
                                  '</div> </div>';
                              }
                              else if (key=='3')
                              {
                                str += '<div class="col-md-12 kt-margin-b-20-tablet-and-mobile">' +
                                  '<div class="form-group">' +
                                  '<label>' + translation(value) + '</label>' +
                                  '<input readonly="" value="انتظار" type="text" class="form-control" id="kt_datepicker_1" readonly="" >' +
                                  '</div> </div>';
                              }
                            }
            ...
            columns: [
              ...
              {
                field: 'is_salary_adapter_on',
                title: '{{__('محول لبنك الرياض')}}',
                sortable: false,
                width: 80,
                template: function (row, index, datatable) {
                if(row.is_salary_adapter_on=='1')
                {
                  return 'نعم'
                }
                else
              ...
              },
              {
                field: 'employer_name',
                width: 50,
                title: '{{__('اسم جهة العمل')}}',
                sortable: false,
              },

              {
                field: 'tenant_total_salary',
                width: 50,
                title: '{{__('راتب المستأجر')}}',
                sortable: false,
              },
      ...
      function translation(key) {
        ...
        if (key == 'owner_identity_number') {
          return 'رقم هوية المالك';
        }
        if (key == 'tenant_name') {
          return 'اسم المستأجر';
        }
        if (key == 'tenant_mobile') {
          return 'جوال المستأجر';
        }   if (key == 'tenant_identity_number') {
          return 'رقم هوية المستأجر';
        }
        if (key == 'tenant_total_salary') {
          return 'راتب المستأجر';
        }
        if (key == 'bank_name') {
          return 'بنك المستأجر';
        }
        ...
        if (key == 'is_salary_adapter_on') {
          return 'الراتب محول لبنك الرياض';
        }
        if (key == 'employer_name') {
          return 'اسم جهة العمل';
        }

        if (key == 'tenant_job_type') {
          return 'نوع العمل';
        }
       return false;
